...
import { ImageCropper } from "./image-cropper" // Import the cropper
...
export function AssetPickerModal({ isOpen, onClose, onSelect }: AssetPickerModalProps) {
    ...
    const [croppingAsset, setCroppingAsset] = useState<Asset | null>(null) // State for cropping
    ...
    const handleSelect = () => {
        if (selectedAsset) {
            // instead of closing immediately, go to crop mode
    ...
    const handleCropComplete = async (blob: Blob) => {
        ...
        try {
            ...
            if (error) {
                ...
                const publicUrl = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/email-assets/${encodeURIComponent(fileName)}`
                onSelect(publicUrl)
                onClose()
            }
        } catch (e) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
            <div className="w-full max-w-2xl mx-4 rounded-lg bg-[#111111] border border-neutral-800 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                {/* Footer - Only show if NOT cropping (cropping has its own buttons) */}
                {!croppingAsset && (
